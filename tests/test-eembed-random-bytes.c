...
unsigned int test_eembed_random_bytes(void)
{
	...
	eembed_random_bytes(buf1, buf_len);
	eembed_random_bytes(buf3, buf_len);
	eembed_random_bytes(buf3 + buf_len, buf_len);
	same = 0;
...
EEMBED_FUNC_MAIN(test_eembed_random_bytes)
